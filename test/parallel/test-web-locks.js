...
describe('Web Locks with worker threads', () => {
  ...
  it('should handle ifAvailable option', async () => {
    await navigator.locks.request('ifavailable-test', async () => {
      ...
      assert.strictEqual(result, null, 'ifAvailable should return null when resource is busy');
      ...
      assert.strictEqual(availableResult, true, 'ifAvailable should succeed when resource is free');
    });
  });
